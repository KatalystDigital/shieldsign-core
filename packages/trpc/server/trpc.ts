...
import { AppError, genericErrorCodeToTrpcErrorCodeMap } from '@shieldsign/lib/errors/app-error';
import { getApiTokenByToken } from '@shieldsign/lib/server-only/public-api/get-api-token-by-token';
import type { TrpcApiLog } from '@shieldsign/lib/types/api-logs';
import type { ApiRequestMetadata } from '@shieldsign/lib/universal/extract-request-metadata';
import { alphaid } from '@shieldsign/lib/universal/id';
...
export const authenticatedMiddleware = t.middleware(async ({ ctx, next, path }) => {
  ...
  // Taken from `authenticatedMiddleware` in `@shieldsign/api/v1/middleware/authenticated.ts`.
