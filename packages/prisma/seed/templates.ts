...
} from '@shieldsign/lib/constants/direct-templates';
import { incrementTemplateId } from '@shieldsign/lib/server-only/envelope/increment-id';
...
export const seedTemplate = async (options: SeedTemplateOptions) => {
  ...
  return await prisma.envelope.create({
    data: {
      ...
      recipients: {
        create: {
          email: 'recipient.1@shieldsign.io',
          name: 'Recipient 1',
