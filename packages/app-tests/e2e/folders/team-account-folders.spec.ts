...
import { DocumentVisibility, FolderType, TeamMemberRole } from '@shieldsign/prisma/client';
import { seedBlankDocument, seedTeamDocuments } from '@shieldsign/prisma/seed/documents';
import { seedBlankFolder } from '@shieldsign/prisma/seed/folders';
import { seedTeamMember } from '@shieldsign/prisma/seed/teams';
...
test('[TEAMS]: can create a document inside a document folder', async ({ page }) => {
  ...
  await fileChooser.setFiles(
    path.join(__dirname, '../../../assets/shieldsign-supporter-pledge.pdf'),
  );

  await page.waitForTimeout(3000);

  await expectTextToBeVisible(page, 'shieldsign-supporter-pledge.pdf');
...
test('[TEAMS]: can pin a document folder', async ({ page }) => {
  ...
  await expect(page.locator('svg.text-shieldsign.h-3.w-3')).toBeVisible();
});
...
test('[TEAMS]: can unpin a document folder', async ({ page }) => {
  ...
  await expect(page.locator('svg.text-shieldsign.h-3.w-3')).not.toBeVisible();
});
...
test('[TEAMS]: can create a template inside a template folder', async ({ page }) => {
  ...
  await page
    ...
    .nth(0)
    .setInputFiles(path.join(__dirname, '../../../assets/shieldsign-supporter-pledge.pdf'));
