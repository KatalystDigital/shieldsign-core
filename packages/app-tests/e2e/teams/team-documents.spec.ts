...
import { seedTeam, seedTeamEmail, seedTeamMember } from '@shieldsign/prisma/seed/teams';
...
test('[TEAMS]: check team documents count with external team email', async ({ page }) => {
  ...
  const teamEmail = `external-team-email-${team.id}@test.shieldsign.io`;
...
test('[TEAMS]: resend pending team document', async ({ page }) => {
  ...
  await page.getByLabel('test.shieldsign.io').first().click();
