...
const TELEMETRY_DISABLED = !!process.env.SHIELDSIGN_DISABLE_TELEMETRY;

const NODE_ID_FILENAME = '.shieldsign-node-id';
...
export class TelemetryClient {
   ...
   *
   * If telemetry is disabled via `SHIELDSIGN_DISABLE_TELEMETRY=true` or credentials are not
   * provided, this will be a no-op.
   */
  public static async start(): Promise<void> {
    if (TELEMETRY_DISABLED) {
      console.log(
        '[Telemetry] Telemetry is disabled. To enable, remove the SHIELDSIGN_DISABLE_TELEMETRY environment variable.',
      );
      return;
    }
  ...
  private async initialize(): Promise<void> {
    ...
    console.log(
      '[Telemetry] Telemetry is enabled. ShieldSign collects anonymous usage data to help improve the product.',
    );
    console.log(
