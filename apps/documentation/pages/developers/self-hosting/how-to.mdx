---
title: How to Self-Host ShieldSign
description: Learn how to self-host ShieldSign on your server or cloud infrastructure using Docker or other deployment methods.
---
...
The following guide will walk you through setting up ShieldSign manually on your server or cloud infrastructure.
...
```bash
git clone https://github.com/ShieldSign/ShieldSign.git
```

### Configure the Environment Variables

Navigate to the `ShieldSign` folder and create a `.env` file from the example `.env.example` file:
...
<Callout type="info">
  If you use a reverse proxy in front of ShieldSign, don't forget to provide the public URL for the
...
The following guide will walk you through setting up ShieldSign using Docker. You can choose between a Docker Compose production setup or a standalone container.

We provide a Docker container for ShieldSign, published on both DockerHub and GitHub Container Registry.

- [DockerHub](https://hub.docker.com/r/ShieldSign/ShieldSign)
- [GitHub Container Registry](https://ghcr.io/ShieldSign/ShieldSign)
...
This setup includes a PostgreSQL database and the ShieldSign application. You will need to provide your own SMTP details using environment variables.

<Steps>

### Download the Docker Compose File

Download the Docker Compose file from the ShieldSign repository - [compose.yml](https://raw.githubusercontent.com/ShieldSign/ShieldSign/release/docker/production/compose.yml).
...
2. Set certificate password securely and generate certificate inside the container:
   ...
   docker exec -e CERT_PASS="$CERT_PASS" -it ShieldSign-production-ShieldSign-1 bash -c "
...
4. Restart the container to apply changes:
   ```bash
   docker-compose restart ShieldSign
   ```
...
3. **Update the volume binding** in the `compose.yml` file:
   ...
   ```yaml
   volumes:
     - /path/to/your/cert.p12:/opt/ShieldSign/cert.p12:ro
   ```

4. **Add certificate configuration** to your `.env` file:
   ```bash
   NEXT_PRIVATE_SIGNING_LOCAL_FILE_PATH=/opt/ShieldSign/cert.p12
...
The command will start the PostgreSQL database and the ShieldSign application containers.

### Access the Application

Access the ShieldSign application by visiting `http://localhost:3000` in your web browser.

</Steps>

### Option 2: Standalone Docker Container

If you prefer to host the ShieldSign application on a specific container provider, use the pre-built Docker image from DockerHub or GitHub's Package Registry. You will need to provide your own database and SMTP host.

<Steps>

### Pull the Docker Image

Pull the ShieldSign Docker image from DockerHub:

```bash
docker pull ShieldSign/ShieldSign
```

Or, pull the image from GitHub Container Registry:

```bash
docker pull ghcr.io/ShieldSign/ShieldSign
```
...
docker run -d \
 ...
 -v /path/to/your/keyfile.p12:/opt/ShieldSign/cert.p12
...
You can access the ShieldSign application by visiting the URL you provided for the `NEXT_PUBLIC_WEBAPP_URL` environment variable in your web browser.

</Steps>

### Advanced Configuration

The environment variables listed above are a subset of those available for configuring ShieldSign. The table below provides a complete list of environment variables and their descriptions.

| Variable                                     | Description                                                                                         |
| -------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| `PORT`                                       | The port on which the ShieldSign application runs. It defaults to `3000`.                           |
...
| `NEXT_PUBLIC_SUPPORT_EMAIL`                  | The support email address displayed to users (default `support@ShieldSign.com`).                    |
...
| `NEXT_PRIVATE_SIGNING_LOCAL_FILE_PATH`       | The path to the key file, default `/opt/ShieldSign/cert.p12`.                                       |
...
| `ShieldSign_DISABLE_TELEMETRY`               | Set to `true` to disable anonymous telemetry (see [Telemetry](#telemetry) section below).           |

## Telemetry

ShieldSign collects anonymous telemetry data to help us understand how the software is being used and improve the product. This telemetry is **enabled by default** for self-hosted instances.

### What We Collect

We collect minimal, privacy-preserving data:

- **App Version**: The version of ShieldSign you are running
...
```bash
ShieldSign_DISABLE_TELEMETRY=true
```
...
Type=simple
User=www-data
WorkingDirectory=/var/www/ShieldSign/apps/remix
...
We offer several alternative deployment methods for ShieldSign if you need more options.
...
[![Deploy to Koyeb](https://www.koyeb.com/static/images/deploy/button.svg)](https://app.koyeb.com/deploy?type=git&repository=github.com/ShieldSign/ShieldSign&branch=main&name=ShieldSign-app&builder=dockerfile&dockerfile=/docker/Dockerfile)
