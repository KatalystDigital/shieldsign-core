...
Navigate to the `shieldsign-core` folder and create a `.env` file from the example `.env.example` file:
...
We provide a Docker container for ShieldSign. You can build the Docker image from the repository and run it with your preferred container hosting provider.
...
Download the Docker Compose file from the ShieldSign repository - [compose.yml](https://raw.githubusercontent.com/KatalystDigital/shieldsign-core/main/docker/production/compose.yml).
...
If you prefer to host the ShieldSign application on a specific container provider, you can build the Docker image from the repository. You will need to provide your own database and SMTP host.
...
```bash
docker build -t shieldsign -f docker/Dockerfile .
```
...
| `NEXT_PUBLIC_SUPPORT_EMAIL`                  | The support email address displayed to users.                                                       |
...
We offer several alternative deployment methods for ShieldSign. You can deploy using your preferred cloud provider's container service with the Docker image.
