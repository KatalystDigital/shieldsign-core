...
Our embedding feature lets you integrate our document signing experience into your own application or website. Whether you're building with React, Preact, Vue, Svelte, Solid, Angular, or using generalized web components, this guide will help you get started with embedding ShieldSign.
...
- Dark mode controls
- The removal of ShieldSign branding from the embedding experience

## How Embedding Works

Embedding with ShieldSign allows you to handle document signing in two main ways:

1. **Using Direct Templates**: Using direct templates you can have an evergreen template that upon completion will create a new document within ShieldSign.
2. **Using a Signing Token**: A more advanced option for those running rich integrations with ShieldSign already. Given a recipients signing token you can embed the signing experience in your application rather than direct the recipient to ShieldSign.
...
| Framework | Package                                                                              |
| --------- | ------------------------------------------------------------------------------------ |
| React     | [@shieldsign/embed-react](https://www.npmjs.com/package/@shieldsign/embed-react)     |
| Preact    | [@shieldsign/embed-preact](https://www.npmjs.com/package/@shieldsign/embed-preact)   |
| Vue       | [@shieldsign/embed-vue](https://www.npmjs.com/package/@shieldsign/embed-vue)         |
| Svelte    | [@shieldsign/embed-svelte](https://www.npmjs.com/package/@shieldsign/embed-svelte)   |
| Solid     | [@shieldsign/embed-solid](https://www.npmjs.com/package/@shieldsign/embed-solid)     |
| Angular   | [@shieldsign/embed-angular](https://www.npmjs.com/package/@shieldsign/embed-angular) |
...
1. **Navigate to your team's templates within ShieldSign**

2. **Click on the direct link template you want to embed**

This will copy the URL to your clipboard, e.g. `https://your-domain.com/d/-WoSwWVT-fYOERS2MI37k`
...
```jsx
import { EmbedDirectTemplate } from '@shieldsign/embed-react';
...
This will typically be done using an API integration where signing tokens are provided as part of the response when creating a document. Alternatively you can manually get a signing link by hovering over a recipient's avatar and clicking their email on a document that you own.

With the signing url on our clipboard we can extract the token the same way we did for the direct link template.

So `https://your-domain.com/sign/lm7Tp2_yhvFfzdeJQzYQF` will become `lm7Tp2_yhvFfzdeJQzYQF`

2. Provide the token to the `EmbedSignDocument` component in your frameworks SDK

```jsx
import { EmbedSignDocument } from '@shieldsign/embed-react';
...
In addition to embedding signing experiences, ShieldSign now supports **embedded authoring**, allowing your users to create documents and templates directly within your application.
