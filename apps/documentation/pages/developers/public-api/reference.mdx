...
> **Note:** This page demonstrates some of the endpoints and how to use them. For the full API reference, see your instance's `/api/v2/openapi` endpoint.
...
This is done by doing a the following GET request:

```sh
curl -X GET "https://your-domain.com/api/v2/envelope/envelope_xxxxxxxxxxxxx" \
...
```sh
curl -X POST "https://your-domain.com/api/v2/envelope/create" \
  -H "Authorization: api_xxxxxxxxxxxxxx" \
...
> **Note:** See the full request and response formats at your instance's `/api/v2/openapi` endpoint.
...
To generate a document from the template, you need to make a POST request to the `/api/v2/envelope/use` endpoint.
...
```sh
curl -X POST "https://your-domain.com/api/v2/envelope/use" \
  -H "Authorization: api_xxxxxxxxxxxxxx" \
  -H "Content-Type: multipart/form-data" \
  -F 'payload={
...
The API allows you to add recipients to an envelope via the `/api/v2/envelope/recipient/create-many` endpoint.

The following is an example of a request which creates 2 new recipients on the envelope.

```sh
curl https://your-domain.com/api/v2/envelope/recipient/create-many \
  --request POST \
...
The API allows you to add fields to an envelope via the `/api/v2/envelope/field/create-many` endpoint.
...
```sh
curl https://your-domain.com/api/v2/envelope/field/create-many \
  --request POST \
  --header 'Authorization: api_xxxxxxxxxxxxxx' \
  --header 'Content-Type: application/json' \
  --data '{
