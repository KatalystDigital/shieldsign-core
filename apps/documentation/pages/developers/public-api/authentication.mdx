...
description: Learn how to create a ShieldSign API key and authenticate your API requests.
---

# API Authentication

ShieldSign uses API keys for authentication. An API key is a unique token that is generated for each client. The client must provide the key whenever it makes an API call. This way, ShieldSign can identify the clients making the requests and authorize their access to the API.
...
![A screenshot of the ShieldSign's dashboard that shows the dropdown menu when you click on your user avatar](/public-api-images/shieldsign-user-dropdown-menu.webp)
...
![A screenshot of the ShieldSign's user settings page that shows the API Tokens page](/public-api-images/api-tokens-page-ShieldSign.webp)
...
![A screenshot of the newly created API token in the ShieldSign dashboard](/public-api-images/shieldsign-api-key-blurred.webp)

Once you've created the token, ShieldSign will display the key on the screen. Make sure to copy the key and store it securely. You won't be able to see the key again once you refresh/leave the page.
...
```bash
curl --location 'https://app.ShieldSign.com/api/v2/envelope/create' \
